...
import com.example.lookcow.Modelo.Dispositivo;
import com.example.lookcow.Modelo.Ganado;
import com.example.lookcow.Modelo.Grupo;
import com.example.lookcow.Modelo.Raza;
import com.example.lookcow.R;
import com.example.lookcow.databinding.ActivityGanadoBinding;
...
public class GanadoActivity extends DrawerBaseActivity {
    ...
    // Spinner de dispositivos, metodo para cargar
    /*@OptIn(markerClass = UnstableApi.class)
    public void LoadDispositivos() {
        List<Dispositivo> dispositivos = new ArrayList<>();
        Log.e("LoadDispositivos", "dispositivo: " + dispositivos);

        databaseReference.child("Dispositivo").addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                if (snapshot.exists()) {
                    for (DataSnapshot ds : snapshot.getChildren()) {
                        // Obtener los valores de los campos
                        String descripcionDispositivo = ds.child("descripcion").getValue(String.class);

                        // Agregar el grupo a la lista
                        dispositivos.add(new Dispositivo(descripcionDispositivo));
                    }

                    // Crear el adaptador para el Spinner
                    ArrayAdapter<Dispositivo> adapter = new ArrayAdapter<>(GanadoActivity.this, android.R.layout.simple_dropdown_item_1line, dispositivos); // Lista de objetos Dispositivo
                    adapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice); // Layout para la vista desplegable
                    spinnerDispositivo.setAdapter(adapter);

                    // Establecer la selección del Spinner según el índice
                    spinnerDispositivo.setSelection(index);

                    spinnerDispositivo.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                        @Override
                        public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                            Dispositivo dispositivoseleccionado = (Dispositivo) parent.getSelectedItem();

                            // Solo actualizar el texto de txtGrupo si no es el valor por defecto (índice 3)
                            if (dispositivoseleccionado != null) {
                                txtDispo.setText(dispositivoseleccionado.getDescripcion()); // Mostrar la descripcion del dispositivo en txtDispositivo
                            } else {
                                txtDispo.setText(""); // Limpiar el texto si el índice es 3
                            }
                        }
                        @Override
                        public void onNothingSelected(AdapterView<?> parent) {
            ...
            public void onCancelled(@NonNull DatabaseError error) {
                // Manejar errores de la conexión a Firebase
                Log.e("LoadDispositivos", "Error al cargar dispositivos: " + error.getMessage());
            }
        });
    ...
    /*private void botonBuscarSexo(){

        btnbusSexo.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {

                if (txtsexo.getText().toString().trim().isEmpty()){
                    ocultarTeclado();
                    Toast.makeText(GanadoActivity.this, "Escriba un Sexo para BUSCAR!!!", Toast.LENGTH_SHORT).show();
                    ...
                    databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            for (DataSnapshot x : snapshot.getChildren()) {
                                ...
                                if (sexo.equalsIgnoreCase(x.child("sexo").getValue().toString())) {
                                    res = true;
                                    ocultarTeclado();
                            ...
                            if (!res){
                                ocultarTeclado();
                                Toast.makeText(GanadoActivity.this, "Sexo ("+sexo+") no encontrado!!", Toast.LENGTH_SHORT).show();
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            for (DataSnapshot tabla: snapshot.getChildren()){
                                ...
                                if (tabla.child("id").getValue().toString().equals(auxid)){
                                    ...
                                    Toast.makeText(com.example.lookcow.Controladora.GanadoActivity.this, "Registro modificado exitosamente!!", Toast.LENGTH_SHORT).show();
                                    txtid.setText("");
                                    txtsexo.setText("");
                                    txtraza.setText("");
                                    txttemp.setText("");
                    ...
                    databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            for (DataSnapshot x : snapshot.getChildren()){
                                ...
                                if (x.child("dispositivo").getValue().toString().equals(dispo)){
                                    res2 = true;
                                    ocultarTeclado();
                                    Toast.makeText(GanadoActivity.this, "El Dispositivo ("+ dispo+") ya existe", Toast.LENGTH_SHORT).show();
                                    break;
                                }
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            for (DataSnapshot x : snapshot.getChildren()) {
                                ...
                                if (auxId.equalsIgnoreCase(x.child("id").getValue().toString())) {
                                    ...
                                    res[0] = true;
                                    Toast.makeText(com.example.lookcow.Controladora.GanadoActivity.this, "ID ( "+auxId+" ) con ( "+raza+" ) encontrado.\nUsted puede eliminar!!", Toast.LENGTH_SHORT).show();

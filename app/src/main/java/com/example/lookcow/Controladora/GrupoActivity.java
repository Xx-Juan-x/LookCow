...
import com.example.lookcow.databinding.ActivityGrupoBinding;
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            for (DataSnapshot tabla : snapshot.getChildren()) {
                                ...
                                if (tabla.child("idGrupo").getValue().toString().equals(auxid)) {
                                    ...
                                    Toast.makeText(com.example.lookcow.Controladora.GrupoActivity.this, "Registro modificado exitosamente!!", Toast.LENGTH_SHORT).show();
                                    txtid.setText("");
                                    txtnombre.setText("");
                                    listarGrupos();
                                    break;
                    ...
                    databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            if (!res && !res2) {
                                ...
                                //Traemos la accion del permiso
                                /*DatabaseReference refspinner = db.getReference().child("Permiso");
                                refspinner.addValueEventListener(new ValueEventListener() {
                                    @Override
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        final List<String> accion = new ArrayList<>();
                                        for (DataSnapshot permi : snapshot.getChildren()) {
                                            String acci = permi.child("accion").getValue(String.class);
                                            accion.add(acci);
                                        }
                                        ArrayAdapter<String> permiAdapter = new ArrayAdapter<String>(GrupoActivity.this, android.R.layout.simple_spinner_item, accion);
                                        permiAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
                                        spinner.setAdapter(permiAdapter);
                                    ...
                                    @Override
                                    public void onCancelled(@NonNull DatabaseError error) {
                                        Toast.makeText(GrupoActivity.this, "Error al conectar a la DB:" + error, Toast.LENGTH_SHORT).show();
                                    }
                                });*/
